...
class DeepScannerEngine:
    ...
    def __init__(self, downloader=None, parser_factory=FinancialDocParser, orchestrator=None, filings_client=None, risk_engine=None, financial_engine=None):
    ...
    def analyze_candidate(self, row: Dict[str, Any], live_filings: bool = True) -> Dict[str, Any]:
        ...
        try:
            ...
            promoter_holding = filing_data.get("promoter_holding_pct") or row.get("promoter_holding_pct")
            promoter_pledge = filing_data.get("promoter_pledge_pct") if filing_data.get("promoter_pledge_pct") is not None else row.get("promoter_pledge_pct")
            promoter_change = filing_data.get("promoter_change_pct") if filing_data.get("promoter_change_pct") is not None else row.get("promoter_change_pct")
            ...
            governance_clean = (forensics.audit_opinion_type.lower().startswith("unmodified") and forensics.contingent_liability_risk.lower().startswith("low") and forensics.related_party_risk.lower().startswith("low") and not forensics.forensic_red_flags and corporate.governance_grade in {"A", "B"})
            ...
            analysis = self.financial_engine.analyze(history, governance_clean=governance_clean, current_price=price, shares_outstanding_cr=shares_cr, target_pe=20.0, margin_of_safety_pct=30.0)
            memo = ai.run_investment_committee(forensics, analysis.ratios, analysis.quality, analysis.valuation)
            return {**row, "status": "ANALYZED", "verdict": analysis.recommendation["verdict"], "decision_reason": analysis.recommendation["reason"], "quality_score": analysis.quality["score_100"], "ai_conviction": memo.conviction_score, "governance_clean": governance_clean, "corporate_risk_score": corporate.risk_score, "governance_grade": corporate.governance_grade, "corporate_risk_flags": ";".join(corporate.risk_flags), "corporate_data_gaps": ";".join(corporate.data_gaps), "promoter_holding_pct": promoter_holding, "promoter_pledge_pct": promoter_pledge, "promoter_change_pct": promoter_change, "shareholding_as_on": filing_data.get("shareholding", {}).get("as_on_date"), "shareholding_xbrl_url": filing_data.get("shareholding", {}).get("xbrl_url"), "filing_source": filing_data.get("source", "NSE"), "pat_cagr_pct": analysis.ratios.get("PAT CAGR (%)"), "revenue_cagr_pct": analysis.ratios.get("Revenue CAGR (%)"), "roce_pct": analysis.ratios.get("ROCE (%)"), "roe_pct": analysis.ratios.get("ROE (%)"), "cfo_pat": analysis.ratios.get("CFO / PAT Quality Ratio"), "debt_equity": analysis.ratios.get("Debt to Equity"), "fair_value": analysis.valuation.get("fair_value"), "buy_below": analysis.valuation.get("buy_below"), "risk_flags": ";".join(forensics.forensic_red_flags or []), "thesis": memo.executive_summary.strip()}
